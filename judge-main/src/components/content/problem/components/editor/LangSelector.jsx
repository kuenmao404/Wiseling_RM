import React from "react";
import {
  Select,
  SelectContent,
  SelectGroup,
  SelectItem,
  SelectLabel,
  SelectTrigger,
  SelectValue,
} from "../../../../elements/ui/Select";
import { editorStorageAtom, setLanguageAtom, useEditor } from "./UseEditor";
import { useAtom, useSetAtom } from "jotai";

export function LangSelector({ languages, pid, ...props }) {
  const updateLanguage = useSetAtom(setLanguageAtom);
  const { langKey: languageKey } = useEditor(pid);

  return (
    <Select
      value={(languages || [])?.find(d => d?.name == languageKey?.name)?.name || "c"}
      defaultValue={languages?.[0]?.name || "c"}
      onValueChange={(lang) => updateLanguage({ lang: (languages || [])?.find(d => d?.name == lang), pid })}
      {...props}
    >
      <SelectTrigger className="w-[180px] text-gray-300">
        <SelectValue placeholder="Select your programming language" />
      </SelectTrigger>
      <SelectContent>
        <SelectGroup>
          <SelectLabel>Languages</SelectLabel>
          {languages.map((lang) => (
            <SelectItem key={lang.name + lang.key} value={lang.name}>
              {lang.name}
            </SelectItem>
          ))}
        </SelectGroup>
      </SelectContent>
    </Select>
  );
}
