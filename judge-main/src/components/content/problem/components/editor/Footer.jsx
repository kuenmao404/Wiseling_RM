...
export function EditorFooter({ onRunClick, onSubmitClick, className, lang, isLoading, pid, ...props }) {
  const setEditorMode = useSetAtom(setEditorModeAtom);
  const setEditorFile = useSetAtom(setEditorFileAtom)
  const { mode, file } = useEditor(pid);
    ...
    <div className={cn("flex gap-1 items-center justify-between bg-[#1a1b26] flex-wrap", className)} {...props}>
      <div className="flex gap-1 items-center justify-start flex-wrap">
        ...
        <UIButton
          ...
          onClick={() => {
            setEditorMode({ mode: (mode == "Normal" ? "Vim" : "Normal"), pid });
          }}
        >
          {mode}
        </UIButton>
        {(file == null || !file?.name) ?
          <Button component="label" variant="outlined" color={`warning`} startIcon={<CloudUpload />}>
            上傳程式碼
            <VisuallyHiddenInput type="file" onChange={(e) => setEditorFile({ file: e.target?.files?.[0], pid })} />
          </Button> :
          <Tooltip title={"點我移除檔案"}>
            <div className="text-white text-xs cursor-pointer" onClick={() => setEditorFile({ pid, file: {} })}>
              {file?.name}
            </div>
          </Tooltip>
        }
      ...
      <div className="flex gap-1 items-center justify-end">
        ...
        <Button variant="contained" color="success" sx={{ textTransform: "none" }} onClick={onSubmitClick} disabled={isLoading}>
          Submit
        </Button>
      </div>
    </div>
