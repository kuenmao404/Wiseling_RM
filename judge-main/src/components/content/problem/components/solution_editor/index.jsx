...
export function Editor({ pid, className, lang, onSubmitClick, ...props }) {
  ...
  const refreshEditorStorage = useAtomCallback(
    useCallback(
      (get) => {
        const { pid: storedPid, langKey: storedLangKey, mode: storedMode, file: storedFile } = get(editorStorageAtom);
