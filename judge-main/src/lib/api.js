...
export default function ({ cmd, method = "GET", type = "json", params = {}, data = {}, header = {}, fileList = [], fileObj = {} }) {
  ...
  if (fileList.length || Object.keys(fileObj)?.length > 0) {
    ...
    Object.keys(fileObj)?.map((key) => {
      if (Array.isArray(fileObj[key])) {
        ...
        fileObj[key] !== null && formData.append(key, fileObj[key])
    })
