...
function Pagination(props) {
  const { total, onChange, defaultValue, value, color, withEllipsis, ellipsisRange, isFixed } = props;
  ...
  const handleClickNext = () => {
    current < total && onChangePage(current + 1)
  };

  const handleClickPrev = () => {
    current > 1 && onChangePage(current - 1)
  };

  const onChangePage = (page) => {
    !!defaultValue ? (setCurrent(page)) : onChange({ current: page })
  }
  ...
  return (
    <div className="whl_pagination">
      <IconButton
        ...
        sx={{ color: current !== 1 ? "#000" : "#00000042" }}
        size="small"
      >
      ...
      <IconButton
        ...
        sx={{ color: current !== total ? "#000" : "#00000042" }}
        size="small"
      >
