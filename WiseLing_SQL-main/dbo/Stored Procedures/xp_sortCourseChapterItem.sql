CREATE   procedure [dbo].[xp_sortCourseChapterItem]
	@courseCID int,
	@cid int,
...
begin
	...
	begin try
		...
		declare @count int = (select count(*) from vd_CourseChapterItem where courseCID = @courseCID and cid = @cid),
				@count_t int = (select count(*) from @tmp),
				@count_join int = (
					select count(*) 
					from vd_CourseChapterItem v, @tmp t
					where  v.courseCID = @courseCID and v.cid = @cid and v.oid = t.id
				)
		...
		begin transaction　[xp_sortCourseChapterItem]
		...
		commit transaction	[xp_sortCourseChapterItem]
	end try
	begin catch
		if XACT_STATE() <> 0 rollback transaction [xp_sortCourseChapterItem]
