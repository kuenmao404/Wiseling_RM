...
namespace WiseLing_API.Lib
{
    public class Param
    {
        ...
        public dynamic getBodyParamByJson(string contentType, DIModel di)
        {
            if (contentType == "application/json")
            {
                dynamic? columnjson = JsonConvert.DeserializeObject(di.RequestBody);
                return columnjson;
            }
        ...
        public async static Task<string> captureRequestBody(HttpContext httpContext)
        {
            // 啟用請求流的重用（允許多次讀取）
            httpContext.Request.EnableBuffering();
